...
from model import TD3CriticNetwork, TD3ActorNetwork
...
def update_network_parameters(actor, target_actor, critic_1, target_critic_1, critic_2,
    ...
    if spiking:
        ...
        for name in actor:
            actor[name] = tau*actor[name].clone() + (1 - tau)*target_actor_dict[name].clone()
...
def run_data_generator(controller_name: str, environment_name: str, config_manager: ConfigManager,
    ...
    actor = TD3ActorNetwork(actor_learning_rate, input_dims, layer1_size, layer2_size, n_actions,
                            'actor')
    target_actor = TD3ActorNetwork(actor_learning_rate, input_dims, layer1_size, layer2_size,
